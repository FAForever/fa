"""
This script converts markdown changelog files into Lua files and generates an overview Lua file.

The script processes a directory of markdown files, extracting YAML front matter and converting
each markdown file into a corresponding Lua file. It also creates an overview file listing all
converted changelogs with their version, name, URL, and path.
"""
...
def convert_changelog_folder(input_dir: Path, output_dir: Path):
    """Converts all markdown files in a directory to Lua files and creates an overview."""
    for markdown_file in input_dir.glob("*.md"):
