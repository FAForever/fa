...
float3 ApplyWaterColor(float3 viewDirection, float terrainHeight, float waterDepth, float3 color)
{
    if (waterDepth > 0) {
        ...
        if (MapUsesAdvancedWater()) {
            ...
            float outAbsorption = 1 - exp(-waterDepth * oneOverCosV);
            ...
            color = lerp(color, waterColor.rgb, outAbsorption * opacity);
        } else {
