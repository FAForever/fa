...
UEFNukeEffectController01 = Class(NullShell) {
    EffectThread = function(self)
        ...
        CreateLightParticle(self, -1, self.Army, 100, 80, 'glow_03', 'ramp_nuke_04')

        -- Create ground decals
        local orientation = RandomFloat(0,2*math.pi)
        CreateDecal(position, orientation, 'Crater01_albedo', '', 'Albedo', 25, 25, 1200, 0, self.Army)
    ...
    CreateHeadConvectionSpinners = function(self)
        ...
        for i = 0, (sides-1) do
            ...
            local proj = self:CreateProjectile('/effects/entities/UEFBillyNukeEffect03/UEFBillyNukeEffect03_proj.bp', x, HeightOffset, z, x, 0, z)
                :SetVelocity(velocity)
            table.insert(projectiles, proj)
        end
