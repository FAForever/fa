...
function AIGetStructureUnitId(aiBrain, structureType)
    local unitId
    for k, v in BuildingTemplates[aiBrain:GetFactionIndex()] do
...
function AIGetMobileUnitId(aiBrain, unitType)
    local unitId
    for k, v in UnitTemplates[aiBrain:GetFactionIndex()] do
...
function AISortMarkersFromStartPos(aiBrain, markerList, maxNumber, tMin, tMax, tRings, tType, tType, position)
    ...
    -- LOG('*AI DEBUG: ARMY 2: LINE 112 of aiutilities.lua, NUM =', repr(num))
    for i = 1, num do
        local lowest = nil
...
function AIGetClosestThreatMarkerLoc(aiBrain, markerType, startX, startZ, threatMin, threatMax, rings, threatType)
    ...
    -- LOG('*AI DEBUG: MARKER: Threat Marker, TYPE = ', markerType, ' Threat = ', threatAtLoc, ' LOC = ', repr(loc))
    return loc, name
end
