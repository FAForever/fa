...
function getBuildingTab()
    ...
    for name, values in btSource do
        buildingTab[name] = {}
        for _, value in values do
            if __blueprints[value] ~= nil then
                table.insert(buildingTab[name], value)
            elseif btSource[value] ~= nil then
                for _, realValue in btSource[value] do
                    if __blueprints[realValue] ~= nil then
...
function buildActionBuilding(name, modifier)
    ...
    for _, value in allValues do
        for _, buildableValue in buildable do
