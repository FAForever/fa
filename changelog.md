...
With the last patch walls now work reliable to block projectiles, even for units that are hiding behind them. This can create almost impenetrable defenses without the use of artillery. Together with the balance team we decided to push the balance changes of walls forward. These changes apply to all wall sections:
...
With the last patch units would no longer rotate towards the best roll off point at the end of construction. Especially for large units (read: naval units) it could look quite odd. We now make an exception to engineers as a wrong oriented engineer at the start of the game can easily slow down your build order with several seconds.
...
They are still part of the objective. It is up to the author of the campaign map to exclude them in the script files if they do not want them to be part of the objective
