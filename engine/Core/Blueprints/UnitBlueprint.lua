...
--- the maximum movement range the Othuy ("lighting storm") has
...
--- the default recoil distance used if a weapon's rack sets `TelescopeBone` but not
--- `TelescopeRecoilDistance`
---@field TelescopeRecoilDistance? number
---@field VeteranHealingMult
...
---
--- Autogenerated unit id from the folder name. When modifying blueprint, this must be defined to
--- the blueprint id it is modifying instead
---@field BlueprintId UnitId
--- autogenerated categories table based on `Categories`
---@field CategoriesHash table<string, true>
--- autogenerated number from `DoNotCollideList`
---@field DoNotCollideListCount number
--- autogenerated table from `DoNotCollideList`
---@field DoNotCollideListHash table<UnparsedCategory, true>
--- autogenerated for unit blueprints generated from a unit with a preset in `EnhancementPresets`
---@field EnhancementPresetAssigned? UnitBlueprintAssignedEnhancementPreset
--- autogenerated faction category from `Categories`
---@field FactionCategory FactionCategory
--- autogenerated later category from `Categories`
---@field LayerCategory LayerCategory
--- autogenerated volume based on `SizeX`, `SizeY`, and `SizeZ`
---@field SizeVolume number
--- autogenerated damage effects size based on `SizeVolume`
---@field SizeDamageEffects number
--- autogenerated damage effects size scale based on `SizeVolume`
---@field SizeDamageEffectsScale number
--- autogenerated tech category from `Categories`
...
---@field RepairConsumeEnergy
---@field RepairConsumeMass
---@field ShowAssistRangeOnSelect
---@field StagingPlatformScanRadius
...
--- Max thrust for the engine. Directly influences speed and acceleration.
---@field MaxEngineThrust number
...
---@field SkipDynamicThreatCalculations?
...
---
--- An autogenerated value representing the threat this unit poses to enemy air units based on
...
--- An autogenerated value representing the threat this unit poses to the enemy due to an
...
--- An autogenerated value representing the threat this unit poses to the enemy units in the
...
--- An autogenerated value representing the threat this unit poses to enemy surface units based on
...
---@field Mesh? FileName
--- the mesh used in the copy entity in the shield that makes shields shine through the fog of war
--- when visible
...
---@field PassOverkillDamage? any unused but transfered to the entity through the shield spec
---@field PersonalBubble
...
---@field TransportShield


---@class UnitBlueprintDisplay
--- shows the abilities of the unit. Multiple abilities could be added like 'string1', 'string2', etc
...
--- The animation that is played when the unit is completed (seen in experimentals, e.g. Megalith - XRL0403)
---@field AnimationActivate string
--- Animation that is played when the unit is building
---@field AnimationBuild string
---@field AnimationBuildRate
--- Multiple animations could be added
---@field AnimationDeath UnitBlueprintAnimationDeath[]
---@field AnimationDeploy
---@field AnimationFinishBuildLand,
---@field AnimationIdle,
---@field AnimationLand,
---@field AnimationLoop,
--- Animation open file is linked here: '/units/UnitID/UnitID_aopen.sca'
---@field AnimationOpen string
--- The animation that is played when the unit is done building
---@field AnimationPermOpen any
--- the animation that is played when the unit surfaces
---@field AnimationSurface any
---@field AnimationTakeOff
---@field AnimationTransform
--- The animation that is played when the unit is upgraded
...
---@field AnimationWater
---@field AttackReticle
---@field AttackReticleSize
---@field AttackReticuleSize
---@field BlinkingLights any
---@field BlinkingLightsFx any
---@field BuildAttachBone any
--- used while building
---@field BuildMeshBlueprint? FileName
--- used while cloaked
---@field CloakMeshBlueprint? FileName
---@field CannonOpenAnimation
...
---@field LoopingAnimation
---@field MaxRockSpeed? number
---@field Mesh MeshBlueprint
---@field MeshBlueprint FileName
--- used if `Wreckage.UseCustomMesh` is set
---@field MeshBlueprintWrecked? FileName
---@field MissileBone?
---@field MotionAdjustment? {Sliderlanding: number, Sliderstatic: number, Yoffsetinmotion: number, Yoffsetlanding: number, slideinmotion: number}
---@field MotionChangeEffects table<MotionChangeType, UnitBlueprintEffects>
---@field MovementEffects table<Layer, UnitBlueprintEffects>   Effects displayed during movement of the unit. Key should be Name of the layer for which the effects are displayed.
...
--- this is the scale of the unit size. This will only change the visual size, when changing this the collision hitbox should be adapted under SizeX, SizeY and SizeZ as well
...
--- contrails shown behind aircraft (in SupCom usually behind tips of wings). Bones are Names of the bones which will have the contrails attached
...
--- Should the treads be scrolled when unit is moving?
...
--- defines the offset of tread marks to bone defined in BoneName. 'n1', 'n2', 'n3' are respectively x, y and z coordinates
...
--- the needed time when producing this unit; This is only a factor of time, not the real time to produce the unit
...
--- you will get bonus if you rebuild this unit over the wreckage of these wreckages. Multible BonusID's could be added
---@field RebuildBonusIds? string[]
--- builder will kill self but provide this amount of health to the unit it is helping
---@field SacrificeMassMult? number
--- builder will kill self but provide this amount of health to the unit it is helping
---@field SacrificeEnergyMult? number
--- energy storage capacity provided by this unit; This gets added to the main resource pool and is not stored in the unit itself
---@field StorageEnergy? number
--- mass storage capacity provided by this unit; This gets added to the main resource pool and is not stored in the unit itself
---@field StorageMass? number
---@field TeleportEnergyCost? number
--- multiply by the unit's energy cost to get the energy per tick cost of teleporting
---@field TeleportEnergyMod? number
---@field TeleportMassCost? number
--- multiply by the unit's mass cost to get the mass per tick cost of teleporting
---@field TeleportMassMod? number
--- multiply by the unit's build time to get the time required to teleport
...
--- defaults to true, adjusts the teleport cost to the base unit
...
---@field Icon
---@field InstantDeathOnSelfDestruct
---@field OrderOverrides
...
---@field UpgradesFrom string
--- What unit, if any, this unit can be upgraded to
---@field UpgradesTo string
---
---@field CommandCapsHash table<CommandCap, true> autogenerated field from `CommandCaps`
...
--- intel types in this table cannot be disabled
---@field ActiveIntel? table<IntelType, boolean>
--- Single unit cloaking
---@field Cloak boolean
--- How far our cloaking goes
---@field CloakFieldRadius number
--- The intel is free. Without this the unit will try drain energy for it's intelligence (radar, sonar, etc) and turn off if you run out
...
--- When you run out of power, how long until this unit turns back on
---@field ReactivateTime? number
--- used by the Eye of Rianne for the vision radius of its scrying
...
--- prefferred height above (-below) land or water surface
...
---
--- autogenerated flag for naval factories
...
-- autogenerated from the unit and projectile blueprints
