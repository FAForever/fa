...
function AddPingGroup(name, blueprintID, type, description)
    ...
    tableInsert(Sync.AddPingGroups, {ID = PingGroup._id, Name = name, BlueprintID = blueprintID, description = description, Type = type})
...
function OnPostLoad()
    ForkThread(function()
        ...
        for _, PingGroup in PingGroups do
            tableInsert(Sync.AddPingGroups, {ID = PingGroup._id, Name = PingGroup.Name, BlueprintID = PingGroup.BlueprintID, Type = PingGroup.Type})
        end
    end)
end