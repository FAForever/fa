    ...
    Navigating = State {
        ...
        Main = function(self)
            ...
            if not path then
                ...
                LOG('Transporting Raid')
                LOG('Distance is '..VDist3(origin, destination))
                LOG('Current Max Transport Distance is '..self.MaxTransportDistance)
                if not NavUtils.CanPathTo(self.MovementLayer, origin, destination) then
                    LOG('We cant path to the position')
                else
                    LOG('We can path to the position')
                end
            ...
            LOG('Path length is '..pathNodesCount)
    ...
    Transporting = State {
        ...
        Main = function(self)
            ...
            if usedTransports then
                ...
                LOG('No transports used')
                if self.MaxTransportDistance < 500 then
                    LOG('Locaiton distance is '..VDist3(self:GetPlatoonPosition(), self.LocationToRaid)..'current MaxTransport distance '..self.MaxTransportDistance)
                    self.MaxTransportDista00000000nce = self.MaxTransportDistance + 100
                    LOG('MaxTransport Distance increased to '..self.MaxTransportDistance)
                end
                self:ChangeState(self.Searching)
            end
            return
                    ...
                    if dx * dx + dz * dz < NavigateDistanceThresholdSquared then
                        if self.RetreatCount < 3 then
                            ...
                            if platoonCount < self.MaxPlatoonCount then
                                ...
                                if merged then
                                    LOG('We have merged with another raid platoon')
                                else
                                    LOG('We have not merged with another raid platoon')
                                end
                            end
                            self:ChangeState(self.Searching)
                        end
