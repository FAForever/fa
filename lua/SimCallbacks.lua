...
local A, B = Vector(0, 0, 0), Vector(0, 0, 0)
...
local DrawSquare = function(x, z, color)
    local A = A 
    local B = B 

    A[1] = x + 0.45
    A[3] = z + 0.45 
    A[2] = GetSurfaceHeight(A[1], A[3])

    B[1] = x + 0.45
    B[3] = z - 0.45 
    B[2] = GetSurfaceHeight(B[1], B[3])
    DrawLine(A, B, color)
...
local DrawCross = function(x, z, color)
    local A = A 
    local B = B 
...
local function CanPathSlope(x,z) 
    local a,b,c,d = GetTerrainHeight(x-0.5,z-0.5),GetTerrainHeight(x-0.5,z+0.5),GetTerrainHeight(x+0.5,z+0.5),GetTerrainHeight(x+0.5,z-0.5) 
    return MathMax(MathAbs(a-b), MathAbs(b-c), MathAbs(c-d), MathAbs(d-a)) <= 0.75 
end
...
Callbacks.ScanTerrain = function (data, units)

    -- receive data and contain it to a grid
    local x = MathFloor(data.x) + 0.5
    local z = MathFloor(data.z) + 0.5
    local radius = data.radius 

    for rz = -radius, radius, 1 do 
        for rx = -radius, radius, 1 do 
            ...
            local t = GetTerrainHeight(lx, lz)
            local s = GetSurfaceHeight(lx, lz)

            -- land node
            if t == s then 
                DrawSquare(lx, lz, "55ff55")
                
                if (not CanPathSlope(lx, lz)) or (not CanPathTerrain(lx, lz)) then
                    DrawCross(lx, lz, "ff5555")
                end

            -- naval 
            elseif t + 1.5 < s then
                DrawSquare(lx, lz, "5555ff")
                
                if (not CanPathTerrain(lx, lz)) then
                    DrawCross(lx, lz, "ff5555")
                end
            -- amph
            else 
                DrawSquare(lx, lz, "FFA500")
