...
---@alias ObjectiveType
---| 'primary'    # Main objective, reguired to complete the mission
---| 'secondary'  # Optional objectives
---| 'bonus'      # Optional objectives, usually hidden, for extra challenges.
---@alias ObjectiveStatus
---| 'complete'   # Objective is finished
---| 'incomplete' # Objective in progress
---@alias ArmyStatistic
---| "Units_Active"     # Own alive units
---| "Units_BeingBuilt" # Own units under construction
---| "Units_Killed"     # Own units lost
---| "Units_History"    # Own units built totals
---| "Enemies_Killed"   # Enemy units killed
---| 'Enemies_MassValue_Destroyed'   # Sum of mass cost of enemry units destroyed
---| 'Enemies_EnergyValue_Destroyed' # Sum of energy cost of enemry units destroyed
---| 'Enemies_Commanders_Destroyed'  # Number of enemy commanders destroyed
...
---@alias ObjectiveExpireResult
---| "complete" # Objective completes when the timer expires
---| "failed"   # Objective fails when the timer expires
---@alias ObjectiveProgressCallback fun(current: integer, required: integer) Function is is called when objective progress changes
...
---@field protected Decals table<string, moho.CDecalHandle> Dictionary of objective decals associated with the target area names
---@field protected UnitMarkers ObjectiveArrow[] Array of unit markers (yellow bouncing arrow) associated with the objective
---@field protected VizMarkers VizMarker[] Array of visibility markers associated with the objective
---@field protected IconOverrides Unit[] Array of units with added objective strategic icon overlay
---@field protected PositionUpdateThreads table<integer, thread> Threads for updating target's position for UI and icon based on available intel
---@field protected ProgressCallbacks ObjectiveProgressCallback[] List of functions to call when the objective progress changes
---@field protected ResultCallbacks ObjectiveResultCallback[] List of functions to call when the objective is completed
---@field protected Timer? thread Timer to finish the objective
...
local Obj = {
    ...
    ---@protected ID for tracking targets
    NextTargetTag = 0,
}
---@param tag string
...
---@protected
function Obj:__init(tag, objType, status, title, description, hidden, target)
    self.Tag = tag
    self.Type = objType
    self.Title = title
...
function Obj:AddProgressCallback(cb)
...
---@param cb ObjectiveProgressCallback
function Obj:RemoveProgressCallback(cb)
...
---@param total? integer Required for the perctange display if its not the same as `required`
function Obj:UpdateProgress(current, required, total)
...
---@param total? integer
function Obj:OnProgress(current, required, total)
...
---@param data? any
function Obj:OnResult(success, data)
    if not self.Active then return end
...
---@param success boolean
function Obj:ManualResult(success)
    self:OnResult(success)
end
---Creates a timer that ends the objective when expired.
---@param time integer In seconds
---@param expireResult? ObjectiveExpireResult Defaults to `failed`
function Obj:AddTimer(time, expireResult)
...
---@return VizMarker
function Obj:CreateVisualMarker(x, z, radius, lifetime)
...
---@param lifetime? number In seconds, defaults to `-1` infinite
---@protected
function Obj:AddUnitVisualMarker(unit, radius, lifetime)
...
---@param lifetime? number In seconds, defaults to `-1` infinite
---@protected
function Obj:AddAreaVisualMarker(area, lifetime)
...
---@return function
---@protected
function Obj:GetTargetDestroyedNotifyCallback(targetTag)
    return function()
...
---@param targetTag integer
---@protected
function Obj:SetupNotify(unit, targetTag)
...
---@param targetTag integer
---@protected
function Obj:SetupFocusNotify(unit, targetTag)
    self.PositionUpdateThreads[targetTag] = ForkThread(function()
        while self.Active do
    ...
    Triggers.CreateUnitDeathTrigger(self:GetTargetDestroyedNotifyCallback(targetTag), unit)
end
---@param unit Unit
...
function Obj:AddUnitTarget(unit, mark, flashVisible, alwaysVisible)
...
---@param unit Unit
---@deprecated
function Obj:AddBasicUnitTarget(unit)
...
---@param alwaysVisible? boolean Creates a visual marker over the area
---@protected
function Obj:AddAreaTarget(area, decal, flashVisible, alwaysVisible)
    self.NextTargetTag = self.NextTargetTag + 1
    local rect = ScenarioUtils.AreaToRect(area)
