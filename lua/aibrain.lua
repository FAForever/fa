...
function CollectCurrentScores()
    -- Initialize the score data structure
    ...
    while true do
        ...
        -- General unit statistics scores 1
        for index, brain in ArmyBrains do
        ...
        -- General unit statistics scores 2
        for index, brain in ArmyBrains do
...
function TransferUnitsOwnership(units, ToArmyIndex)
    ...
    for k,v in units do
        ...
        -- as well, so we only want the top level units to be given. Also, don't allow commanders to be given.
        ...
        local unitKills = unit:GetStat('KILLS', 0).Value   -- Also takes care of the Veterancy level
        ...
        if bp.Physics.FuelUseTime and bp.Physics.FuelUseTime > 0 then   -- Going through the BP to check for fuel
            ...
            hasFuel = true                                              -- because some buildings say they use fuel
        end
...
-- This function update the reclaim income statistic
function UpdateReclaimStat()
    while true do
        for index, brain in ArmyBrains do
            local reclaimedMass     = brain:GetArmyStat("Economy_Reclaimed_Mass", 0.0).Value
...
-----------------------------------------------
-- This class handles all Human brain functions
----------------------------------------------
AIBrain = Class(moho.aibrain_methods) {
    ...
    InitializeEconomyState = function(self)
        -- This is called very early, so ensure statistics exist
    ...
    CalculateScore = function(self)
        ...
        -- Helper variable to make equation clearer
    ...
    SetUnitIntelTable = function(self, unit, reconType, val)
        if unit and not unit:IsDead() and IsUnit(unit) the
            local UnitId = unit:GetEntityId()
    ...
    -- Called when reconnaissance data changes for enemy units (e.g. A unit comes into line of sight)
    -- Parameters:
    --      blip: the unit (could be fake) in question
    --      type: 'LOSNow', 'Radar', 'Sonar', or 'Omni'
    --      val: true or false
    -- Calls callback function with blip it sa
    ...
    OnDestroy = function(self)
        if self.BuilderManagers then
            ...
            for k,v in self.BuilderManagers d
    ...
    OnDefeat = function(self)
        ...
        -- Score change, we send the score of all other players
        for index, brain in ArmyBrains do
    ...
    InitializeSkirmishSystems = function(self)
        ...
        -- Economy monitor for new skirmish - stores our economy over time to get trend over 10 seconds
        self.EconomyData = {}
        ...
        -- Add default main location and set up the builder managers
        self.NumBases = 1
    ...
    -- Used to get rid of nil table entries for Sorian AI
                            ...
                            if v.PrimaryFactories[typev] then
                                ...
                                if numBuildOrders and numBuildOrders == 0 then
                                    ...
                                    for kp, vp in platoonList[typev] do
                                        -- Don't try to build things that are higher priority than 0
    ...
    PBMBuildNumFactories = function (self, template, location, pType, factory)
        ...
        -- Handle squads with programmatic build quantity
        squad = 3
    ...
    BaseMonitorCheck = function(self)
        ...
        if table.getn(vecs) > 0 then
            ...
            -- Remove any points that we don't monitor any more
            for k,v in self.BaseMonitor.BaseMonitorPoints do
                local found = false
                ...
                -- If point not in list and the number units around the point is small
    ...
    ParseIntelThreadSorian = function(self)
        ...
        while true do
            ...
            numchecks = 0
            -- Get rid of outdated intelligence
            ...
            -- Rebuild intelligence table if there was a change
            if changed then
            ...
            --Handle intelligence data if there was a change
            if changed then
    ...
    AddInitialEnemyThreat = function(self, amount, decay)
        ...
        if ScenarioInfo.Options.TeamSpawn == 'fixed' the
            -- Spawn locations were fixed. We know exactly where our opponents are
            for i=1,12 do
                local token = 'ARMY_' .. i
                local army = ScenarioInfo.ArmySetup[token]
    ...
    BuildScoutLocations = function(self)
        ...
        if not aiBrain.InterestList then
            ...
            if ScenarioInfo.Options.TeamSpawn == 'fixed' then
                ...
                -- If the start location is not ours or an ally's, it is suspiciou
                local starts = AIUtils.AIGetMarkerLocations(aiBrain, 'Start Location')
                for _,loc in starts do
...
-----------------------------------------
-- Sorian AI's main functionality section
-----------------------------------------
