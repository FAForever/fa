...
Manager = {
            ...
            for numTable, conditionTable in action.ActionConditions do
                ...
                for numCond, cond in conditionTable do
                    if cond == spec.Name then
                        inserted = true
                        tableInsert(spec.WhichActions, action.Name)
                        break
                    end
                end
            end
        ...
        for numTable, conditionTable in spec.ActionConditions do
            for numCond, cond in conditionTable do
                ...
                if shouldInsert then
                    tableInsert(self.TriggerList[cond].WhichActions, spec.Name)
                end
            end
        end
    ...
    Area = function(self, spec)
        ...
        if not params.Rectangle or tableGetn(params.Rectangle) <= 0 then
    ...
    ArmyStats = function(self, spec)
        ...
        if not found then
            tableInsert(params.Brain.TriggerList, brainSpec)
        end
        aibrain_methodsSetArmyStatsTrigger(params.Brain, params.StatName, spec.Name,
                                         params.CompareType,
    ...
    EconStats = function(self, spec)
        ...
        aibrain_methodsSetArmyStatsTrigger(params.Brain, statName, spec.Name, params.CompareType, params.Number)
        return true
    end,
    ...
    DamageUnits = function(self, action, conditionList)
        ...
        if params.Unit then
            tableInsert(params.Group, params.Unit)
        end
        ...
        if params.UnitName then
            if ScenarioInfo.UnitNames[armyIndex][params.UnitName] then
                tableInsert(params.Group, ScenarioInfo.UnitNames[armyIndex][params.UnitName])
            else
    ...
    TriggerFire = function(self, name, ...)
        ...
        if trigger.Active then
            ...
            if arg['n'] > 0 then
                local i = 1
                while i <= arg['n'] do
                    tableInsert( trigger.CallbackParameters, arg[i])
                    i = i + 1
                end
            end
        ...
        for actNum, action in actionTable do
            ...
            elseif action.ActionType == 'Give Mass' then
                for pNum, pVars in action.Parameters do
                    aibrain_methodsGiveResource(GetArmyBrain[pVars.ArmyName], 'MASS', pVars.Amount)
                end
            elseif action.ActionType == 'Give Energy' then
                for pNum, pVars in action.Parameters do
                    aibrain_methodsGiveResource(GetArmyBrain[pVars.ArmyName], 'ENERGY', pVars.Amount)
                end
