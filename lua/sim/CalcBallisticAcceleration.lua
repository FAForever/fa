...
CalculateBallisticAcceleration = function (weapon, proj, MuzzleSalvoSize, MuzzleSalvoDelay)
    ...
    if bomber_table[entityId] then
        ...
        bp = weapon:GetBlueprint()
        if bp.DropBombShort then
            -- Deliberately launch Torpedo bombers short
            alpha = (bp.DropBombShort) * (pos_proj[2] - GetSurfaceHeight(pos_target[1], pos_target[3]))
        ...
        local offset = (MuzzleSalvoSize - 1) * MuzzleSalvoDelay * vhorz_launcher * 0.33
