...
--**  Authored by Forged Alliance Forever (the association) - this file is shared with the GPL V3 license applied to it
--*********************************************************************************************************************
...
LabelTree = ClassSimple {
    ...
    Populate = function(self, rCache)
        ...
        if uniform then 
            ...
            local hc = 0.5 * self.c
            self.children = {
                tl = LabelTree(self.bx, self.bz, hc, self.ox, self.oz),
                tr = LabelTree(self.bx, self.bz, hc, self.ox + hc, self.oz),
                bl = LabelTree(self.bx, self.bz, hc, self.ox, self.oz + hc),
                br = LabelTree(self.bx, self.bz, hc, self.ox + hc, self.oz + hc)
            }

            for k, child in self.children do
                child:Populate(rCache)
            end
        end
    end,
    ...
    PointInside = function(self, position)
        if position[1] > self.bx and position[1] < self.bx + self.c then
            if position[3] > self.bz and position[3] < self.bz + self.c then
                return true
            end
        end
...
function ProcessMap()
    ...
    local cells = size / amount

    LOG(string.format("Amount: %d", amount))
    LOG(string.format("Size: %d", size))
    LOG(string.format("Cells: %d", cells))

    local tCache, dCache, daCache, pxCache, pzCache, pCache, bCache, rCache = InitCaches(cells)


    local block = LabelTree(2 * cells, 2 * cells, cells)
    block:Populate(tCache, dCache, pxCache, pzCache, bCache)
    ...
    for z = 0, amount - 1 do
        ...
        for x = 0, amount - 1 do
            ...
            trees[z][x]['land'] = LabelTree(z * cells, x * cells, cells)
            trees[z][x]['naval'] = LabelTree(z * cells, x * cells, cells)
            trees[z][x]['hover'] = LabelTree(z * cells, x * cells, cells)
            trees[z][x]['amph'] = LabelTree(z * cells, x * cells, cells)

            -- pre-computing the caches is irrelevant layer-wise, so we just pick 
