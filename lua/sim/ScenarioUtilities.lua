...
---Data loaded from the map's `_save.lua` on Sim init.
---
---Generated by the map editor.
---@class Scenario
---@field Areas table<AreaName, Area>
---@field Armies table<ArmyName, ScenarioArmy>
---@field Chains table<ChainName, MarkerChain>
---@field MasterChain {['_MASTERCHAIN_']: {['Markers']: table<string, Markers>}}
---@field Orders table<OrderName, ScenarioOrder[]> Order queue for units / platoons
---@field Platoons table<PlatoonName, PlatoonTemplate>
---@field Props table Empty, props are saved in the binary .scmap file
---@field next_area_id string Next ID for the object name in the map editor
---@field next_army_id string Next ID for the object name in the map editor
---@field next_group_id string Next ID for the object name in the map editor
---@field next_platoon_id string Next ID for the object name in the map editor
---@field next_queue_id string Next ID for the object name in the map editor
---@field next_unit_id string Next ID for the object name in the map editor

---@alias MarkerName string Name of the Marker in the `_save.lua` file
---@alias ChainName string Name of the Chain in the `_save.lua` file
---@alias AreaName string Name of the Area in the `_save.lua` file
---@alias OrderName string Name of the Order in the `_save.lua` file
---@alias PlatoonName string Name of the Platoon in the `_save.lua` file
---@alias UnitName string Name of the Unit in the `_save.lua` file
---@alias GroupName string Name of the Group in the `_save.lua` file
---@alias ArmyName string Name of the Army in the `_save.lua` file
---@alias ScenarioBuilderName string Name of the Platoon Builder in the `_save.lua` file

---@class Area
---@field rectangle Rectangle

---Contains a list of marker names, used mainly by campaign AI to define routes
---@class MarkerChain
---@field Markers MarkerName[]

---Marker data loaded from the `save.lua` file
---@class Marker
...
---@class CameraMarker: Marker
---@field zoom number
---@field canSetCamera boolean
---@field canSyncCamera boolean
---@field editorIcon string

---@class RallyMarker: Marker
---@field hint boolean

---@alias Markers Marker | ResourceMarker | CameraMarker | RallyMarker

---Order's data from the map's `_save.lua` generated by the map editor.
---@class ScenarioOrder
---@field type string
...
---Unit's data from the map's `_save.lua` generated by the map editor.
---@class ScenarioUnit
---@field type BlueprintId
---@field orders OrderName
---@field platoon PlatoonName
---@field Position {[1]: number, [2]: number, [3]: number}
---@field Orientation {[1]: number, [2]: number, [3]: number}

---Group of units' data from the map's `_save.lua` generated by the map editor.
---@class ScenarioGroup
---@field type "GROUP"
---@field orders OrderName
---@field platoon PlatoonName
---@field Units table<UnitName|GroupName, ScenarioUnit|ScenarioGroup>

---Army's data from the map's `_save.lua` generated by the map editor.
---@class ScenarioArmy
---@field personality string
---@field plans FileName Path to the AIBrain's plan file.
---@field color integer
---@field faction Faction
---@field Economy {mass: number, energy: number} Starting economy
---@field Alliances table<ArmyName, AllianceType> Starting alliances
---@field Units ScenarioGroup
---@field PlatoonBuilders {next_platoon_builder_id: integer, Builders: table<ScenarioBuilderName, PBMPlatoonBuilder>}

local TableGetn = table.getn

---@deprecated
---@param enabled boolean
---@param unitThreshold? integer
function EnableLoadBalance(enabled, unitThreshold)
    if not ScenarioInfo.LoadBalance then
        ScenarioInfo.LoadBalance = {
            Accumulator = 0,
            Enabled = false,
            ---@type {[1]: BaseManager, [2]: GroupName, [3]: boolean}
            SpawnGroups = {},
            ---@type {[1]: string, [2]: GroupName, [3]: UnitFormations, [4]: fun(platoon: Platoon)}
            PlatoonGroups = {},
        }
    end
...
function LoadBalanceThread()
    ...
    while not table.empty(ScenarioInfo.LoadBalance.SpawnGroups) do
        ---@type BaseManager, GroupName, boolean
    ...
    while not table.empty(ScenarioInfo.LoadBalance.PlatoonGroups) do
        ---@type string, GroupName, UnitFormations, fun(platoon: Platoon)
...
---
---One marker can be in multiple chains, this function returns first chain is finds
...
---@param tblNode? ScenarioGroup
---@param tblResult? ScenarioUnit[]
---@return table<UnitName, ScenarioUnit>
...
---@param tblNode ScenarioGroup|ScenarioUnit
---@param tblResult PlatoonName[]
---@return PlatoonName[]
...
---@param strUnit UnitName
---@param tblNode? ScenarioGroup
---@return ScenarioUnit|nil
function FindUnit(strUnit, tblNode)
    if nil == tblNode then
        return nil
    end

    ---@type ScenarioUnit|nil
    local tblResult = nil
...
---Creates a unit `strUnit` for army `strArmy` from the Scenario _save.lua data.
---
---Reference to the unit is saved into `ScenarioInfo.UnitNames[armyIndex][strUnit]` global table
---@param strArmy ArmyName
...
function CreateArmyUnit(strArmy, strUnit)
    ...
    if tblUnit.platoon ~= nil and tblUnit.platoon ~= '' then
        local platTemplate = Scenario.Platoons[tblUnit.platoon]
        local i = 3
        while i <= table.getn(platTemplate) do
            if tblUnit.type == platTemplate[i][1] then
                platoon = brain:MakePlatoon('None', 'None')
                brain:AssignUnitToPlatoon(platoon, unit, platTemplate[i][4], platTemplate[i][5])
                break
            end
            i = i + 1
        end
...
---@param strGroup GroupName
---@param tblNode? ScenarioGroup
---@return ScenarioGroup|nil
function FindUnitGroup(strGroup, tblNode)
    if nil == tblNode then
        return nil
    end

    ---@type ScenarioGroup|nil
    local tblResult = nil
    for strName, tblData in pairs(tblNode.Units) do
        if 'GROUP' == tblData.type then
...
---@param ... any
---@return Unit[]
---@return table<UnitName, Unit>
---@return table<PlatoonName, Platoon>
function CreateArmySubGroup(strArmy, strGroup, ...)
    local tblNode = Scenario.Armies[strArmy].Units
    ---@type Unit[]?
    local tblResult = {}
    ---@type table<UnitName, Unit>?
    local treeResult = {}
    ---@type table<PlatoonName, Platoon>?
    local platoonList = {}
    local brain = GetArmyBrain(strArmy)--[[@as CampaignAIBrain]]
    if not brain.IgnoreArmyCaps then
        SetIgnoreArmyUnitCap(brain:GetArmyIndex(), true)
    ...
    for strName, tblData in pairs(tblNode.Units) do
        if 'GROUP' == tblData.type then
            if strName == strGroup then
                if arg['n'] >= 1 then
                    platoonList, tblResult, treeResult = CreateSubGroup(tblData, strArmy, unpack(arg))
                else
                    platoonList, tblResult, treeResult = CreatePlatoons(strArmy, tblData)
                end
            end
        end
    end
...
---@return Unit[]|nil units
---@return Unit|nil commander
...
---Call this from the map script to set up skirmish game.
---@see InitializeScenarioArmies for starting a mission-like environment
...
---Call this from the map script to set up mission-like envvironment.
---@see InitializeArmies for starting a classic skirmish game
...
---@param strGroup GroupName
---@return Platoon
---@return table<PlatoonName, Platoon> platoonList
---@return Unit[] units
---@return table<UnitName, Unit> unitList
...
---@param strArmy ArmyName
---@param tblNode? ScenarioGroup
---@param tblResult? Unit[]
---@param platoonList? table<PlatoonName, Platoon>
---@param currPlatoon? PlatoonName
---@param treeResult? table<UnitName, Unit>
...
function CreatePlatoons(strArmy, tblNode, tblResult, platoonList, currPlatoon, treeResult, balance)
    ...
    local brain = GetArmyBrain(strArmy)
    local armyIndex = brain:GetArmyIndex()
    ---@type PlatoonTemplate|nil
    local currTemplate
    ---@type integer|nil Number of field sof the Platoon template
    local numRows
    ---@type PlatoonName|nil
    local reversePlatoon
    ---@type integer|nil
    local reverseRows
    local reverseTemplate
    if nil ~= tblNode.platoon and '' ~= tblNode.platoon and tblNode ~= currPlatoon and not platoonList[tblNode.platoon] then
    ...
    for strName, tblData in pairs(tblNode.Units) do
        if 'GROUP' == tblData.type then
            ...
            local unit = CreateUnitHPR(tblData.type,
                strArmy,
                tblData.Position[1], tblData.Position[2], tblData.Position[3],
                tblData.Orientation[1], tblData.Orientation[2], tblData.Orientation[3]
            )
            ...
            if unitNames then
                unitNames[strName] = unit
            end
            unit.UnitName = strName
            local platName = tblData.platoon
            if platName ~= nil and platName ~= '' and platName ~= currPlatoon then
                reversePlatoon = currPlatoon
                reverseRows = numRows
                reverseTemplate = currTemplate
                
                local platoon = platoonList[platName]
                if not platoon then
                    currTemplate = Scenario.Platoons[platName]
                    platoon = brain:MakePlatoon('', currTemplate[2])
                    platoon.squadCounter = {}
                    platoonList[platName] = platoon
                end
                currPlatoon = platName
                currTemplate = Scenario.Platoons[currPlatoon]
                numRows = TableGetn(currTemplate)
            end
            ...
            if currPlatoon then
                ...
                while i <= numRows and not inserted do
                    local platoon = platoonList[currPlatoon]
                    if platoon.squadCounter[i] == nil then
                        platoon.squadCounter[i] = 0
                    end
                    ---@cast currTemplate -nil
                    ...
                    if tblData.type == bpId and platoon.squadCounter[i] < max then
                        ...
                        brain:AssignUnitToPlatoon(platoon, unit, squad, formation)
                        inserted = true
                    end
                    i = i + 1
                end
...
---@param strArmy ArmyName
---@param strGroup GroupName
---@param wreckage? boolean If `true` spawns the units as wreckages and the function returns `nil`
---@param balance? boolean
---@return Unit[] units
---@return table<UnitName, Unit> unitList
---@return table<PlatoonName, Platoon> platoonList
---@overload fun(strArmy: ArmyName, strGroup: GroupName, wreckage: true, balane?: boolean)
function CreateArmyGroup(strArmy, strGroup, wreckage, balance)
    ...
    -- When tblResult isn't nil, the other returned value from CreatePlatoon also exist
    ---@cast tblResult -nil
    ---@cast treeResult -nil
    ---@cast platoonList -nil
    if wreckage then
...
-- Returns tree of units created by the editor.
---@param strArmy ArmyName
---@param strGroup GroupName
---@return table<UnitName, Unit>
---@return Unit[]
...
---@param OnFinishedCallback fun(platoon: Platoon)
function CreateArmyGroupAsPlatoonBalanced(strArmy, strGroup, formation, OnFinishedCallback)
    ScenarioInfo.LoadBalance.Accumulator = 0
    local platoon = CreateArmyGroupAsPlatoon(strArmy, strGroup, formation, nil, nil, true)

    OnFinishedCallback(platoon)
end
...
function CreateArmyGroupAsPlatoon(strArmy, strGroup, formation, tblNode, platoon, balance)
    ...
    tblNode = tblNode or FindUnitGroup(strGroup, Scenario.Armies[strArmy].Units)
    if not tblNode then
        error('*SCENARIO UTILS ERROR: No group named- ' .. strGroup .. ' found for army- ' .. strArmy, 2)
    elseif not formation then
    ...
    platoon = platoon or brain:MakePlatoon('', '')
    local armyIndex = brain:GetArmyIndex()
...
---@param strGroup GroupName
---@param tblNode? ScenarioGroup
---@param unitGroup? ScenarioUnit[]
---@return ScenarioUnit[]
function FlattenTreeGroup(strArmy, strGroup, tblNode, unitGroup)
    tblNode = tblNode or FindUnitGroup(strGroup, Scenario.Armies[strArmy].Units)
...
---Defines a starting position for the army
---Either by reading from marker by the `strArmy` or generating a random position.
---@param strArmy ArmyName
...
---Loads the army plan from the map `Scenario`
---@param strArmy ArmyName
...
---@param strArmy ArmyName
---@param builderData PBMPlatoonBuilder
...
---@param builderEdit PBMPlatoonBuilderTask
---@param builderData PBMPlatoonBuilder
...
---@param buildName string|Scenario Builder name or generated Scenario
---@param strArmy ArmyName
---@param builderData PBMPlatoonBuilder
