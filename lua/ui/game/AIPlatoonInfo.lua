...
AIPlatoonInfoUI = ClassUI(Window) {
    ...
    __init = function(self, parent)
        Window.__init(self, parent, "AI Platoon information", false, false, false, true, false, "AIPlatoonInfo2", {
            Left = 10,
            Top = 300,
            Right = 310,
            Bottom = 525
        ...
        ---@param data AIPlatoonDebugInfo
            function(data)
                if not IsDestroyed(self) then
                    self.Info = data
                    self.NumberOfElements = table.getn(data.Info.DebugMessages)
                    self:CalcVisible()
                end
    ...
    CalcVisible = function(self)
        for k = 1, self.NumberOfUIElements do
            ...
            if index <= self.NumberOfElements then
                self.UIElements[k]:SetText(self.Info.Info.DebugMessages[index])
            else
