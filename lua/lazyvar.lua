...
--- Creates a LazyVar that derives its value from `source` and subscribes your
--- `onDirty` handler to changes in `source`. This is the safe way to observe a
--- LazyVar you don't own: the new LazyVar gets its own `OnDirty` slot, so
--- assigning your handler can never stomp another subscriber on `source`.
---
--- How it differs from `Create`:
---
--- * `Create(value)` allocates a LazyVar holding a static initial **value**
---   (or, with no argument, `0`). It has no compute function, no upstream
---   dependency, and no way to be notified when anything else changes. If you
---   pass a LazyVar or a function, it is stored verbatim as the cached value —
---   not interpreted as a dependency.
---
--- * `Derive(source, onDirty)` allocates a new LazyVar, hangs `onDirty` on it,
---   and calls `:Set(function() return source() end)`. The first evaluation
---   reads `source` inside an `EvalContext`, which registers the derived
---   LazyVar in `source`'s used-by table (see the `__call` metamethod). From
---   then on, any `source:Set(...)` propagates a dirty signal to the derived
---   LazyVar and fires your handler.
---
--- The handler receives the derived LazyVar; call it (`lv()`) to read the
--- current value, which transparently re-evaluates the `source()` compute.
---
--- Destroy the returned LazyVar when the owner is torn down so its `OnDirty`
--- can't fire into a dead receiver.
---
--- @generic T
--- @param source fun(): T | LazyVar<T>    # upstream value provider
--- @param onDirty fun(lv: LazyVar<T>)     # fires whenever `source` changes
--- @return LazyVar<T>
function Derive(source, onDirty)
    local lv = Create()
    lv.OnDirty = onDirty
    lv:Set(function() return source() end)
    return lv
end
