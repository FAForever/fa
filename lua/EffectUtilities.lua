...
function CalculateSliceBeamStats(builder, unitBeingBuilt)
    local bp = unitBeingBuilt:GetBlueprint()
    local x, y, z = unpack(unitBeingBuilt:GetPosition())
    y = y + (bp.Physics.MeshExtentsOffsetY or 0)

    -- Determine beam positioning on build cube
    local ox, oy, oz = CalculateBuildCubeSize(bp)
    ...
    return oy, endVec1, endVec2
end

-- Call with flipped = true to have your beam start the other side
function SliceBeamLoop(builder, unitBeingBuilt, BeamEndEntity, flipped)
    local oy, endVec1, endVec2 = CalculateSliceBeamStats(builder, unitBeingBuilt)
    ...
    local flipDirection = flipped

    -- Warp our projectile back to the initial corner and lower based on build completeness
    while not builder:BeenDestroyed() and not unitBeingBuilt:BeenDestroyed() and not BeamEndEntity:BeenDestroyed() do
...
function RandomisedSliceBeamLoop(builder, unitBeingBuilt, BeamEndEntity)
...
-- UEF Engineers building things
function CreateSliceBeam(builder, unitBeingBuilt, BuildEffectBones, BuildEffectsBag, flipped)
    local army = builder:GetArmy()
    ...
    for i, BuildBone in BuildEffectBones or {} do
    ...
    SliceBeamLoop(builder, unitBeingBuilt, BeamEndEntity, (flipped or false))
end

-- Used by UEF factories and by UEF engies repairing/assisting things
function CreateDefaultBuildBeams(builder, unitBeingBuilt, BuildEffectBones, BuildEffectsBag)
    ...
    local beamThread = builder:ForkThread(RandomisedSliceBeamLoop, unitBeingBuilt, BeamEndEntity)
end
