...
function EnemyToAllyRatioLessOrEqual(aiBrain, num)
    ...
    for k,v in ArmyBrains do
        if v.Status ~= "Defeat" and not ArmyIsCivilian(v:GetArmyIndex()) and IsEnemy(v:GetArmyIndex(), aiBrain:GetArmyIndex()) then
            enemies = enemies + 1
        elseif v.Status ~= "Defeat" and not ArmyIsCivilian(v:GetArmyIndex()) and IsAlly(v:GetArmyIndex(), aiBrain:GetArmyIndex()) then
            allies = allies + 1
        end
    end
...
function AIOutnumbered(aiBrain, bool)
    ...
    for k,v in ArmyBrains do
        if v.Status ~= "Defeat" and aiBrain:GetArmyIndex() ~= v:GetArmyIndex() and not ArmyIsCivilian(v:GetArmyIndex()) then
