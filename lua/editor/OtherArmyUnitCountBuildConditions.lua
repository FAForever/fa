...
function BrainGreaterThanNumCategory( aiBrain, targetBrain, numReq, category )
    ...
    local numUnits = aibrain_methodsGetCurrentUnits(testBrain, category)
...
function FocusBrainBeingBuiltOrActiveCategoryCompare( aiBrain, numReq, categories, compareType )
    ...
    for k,v in categories do
        num = num + aibrain_methodsGetBlueprintStat(testBrain, 'Units_BeingBuilt', v)
        num = num + aibrain_methodsGetBlueprintStat(testBrain, 'Units_Active', v)
    end
