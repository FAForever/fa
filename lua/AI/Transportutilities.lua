...
function CheckTransportPool( aiBrain )
    ...
    local gametime = MathFloor(GetGameTimeSeconds())
	...
	if MovementLayer == 'Land' or MovementLayer == 'Amphibious' then
		...
		if surthreat > mythreat or airthreat > airthreatMax then
            ...
            if (mythreat * 1.5) > surthreat then
                ...
                if TransportDialog then
                    LOG("*AI DEBUG "..aiBrain.Nickname.." "..repr(self.BuilderName).." carried by "..transportplatoon.BuilderName.." seeking alternate landing zone within "..markerrange.." of destination "..repr(destination))
                end
	...
	if PlatoonExists( aiBrain, transports ) then
		...
		for _,v in GetPlatoonUnits(transports) do
			...
			if not v.Dead then
				...
				local slots = TableCopy( aiBrain.TransportSlotTable[v.BlueprintID] )
