...
NavalOpAI = Class(OpAI) {
    Create = function(self, brain, location, name, data)
        ...
        local maxFrigates = data.MaxFrigates or ( (tableGetn(bManager:GetAllBaseFactories(categories.NAVAL)) or 1) * maxMultiplier )
        if maxFrigates == 0 then maxFrigates = 1 end
        local minFrigates = data.MinFrigates or ( (tableGetn(bManager:GetAllBaseFactories(categories.NAVAL)) or 1) * minMultiplier )
