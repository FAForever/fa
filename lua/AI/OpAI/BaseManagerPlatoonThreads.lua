...
local categoriesCONSTRUCTION = categories.CONSTRUCTION
local categoriesAIR = categories.AIR
local categoriesLAND = categories.LAND
local categoriesCOMMAND = categories.COMMAND
local categoriesSUBCOMMANDER = categories.SUBCOMMANDER
local categoriesSTRUCTURE = categories.STRUCTURE

---@class BMAssistPlatoonData
---@field Time integer|nil Min time in ticks the engineer will try to find and assist things. After this time it can find something else to do.
...
--#region Conditional Builds

---@class ConditionalBuildUnit: Unit
---@field KeepAlive boolean|nil # Flag that indicates the unit should be rebuilt if it
---@field ConditionalBuild ConditionalBuildEntry # The conditional build entry that spawned this unit
---@field ConditionalBuildManager ConditionalBuildManager # The conditional build data that spawned this unit
---@field UnitName string Unit name from the editor save.lua file
---@field BaseName string Name of the BaseManager that spawned this unit
---@field AddedCompletionCallback boolean|nil

--- If there is a conditional build that this engineer can tackle, then this function will return true
--- and the base managers ConditionalBuildManager.Index will have the index of ConditionalBuildTable stored in it
...
function CanConditionalBuild(singleEngineerPlatoon)
    ...
    local cbManager = bManager.ConditionalBuildManager
    local cbTable = bManager.ConditionalBuildTable
    if cbManager.IsBuilding then
        -- If there's a build in progress but the unit is dead, reset the variables.
        if cbManager.Unit:BeenDestroyed() then
            local selectedBuild = cbTable[cbManager.Index]
            -- If we're not supposed to retry, then remove from the conditional build list
            if not selectedBuild.data.Retry then
                TableRemove(cbTable, cbManager.Index)
            end
            cbManager:Reset()
        else
            return false
    ...
    if cbManager.IsInitiated then
        ...
        if cbManager.MainBuilder and not cbManager.MainBuilder.Dead then
            return false
        end
    end

    -- Are there no conditional builds?
    if TableEmpty(cbTable) then
        return false
    end
    ...
    table.foreachi(cbTable, function(index, build)
        ---@cast index +integer, -string
...
function ConditionalBuilderDead(engineer)
    ...
    bManager.ConditionalBuildManager:DecrementAssisting()
end

---@param conditionalUnit ConditionalBuildUnit
function ConditionalBuildDied(conditionalUnit)
    local aiBrain = conditionalUnit.Brain --[[@as CampaignAIBrain]]
...
local function unlockConditionalBuildThread(name, seconds)
    WaitSeconds(seconds)
    ScenarioInfo.ConditionalBuildLocks[name] = false
end
...
function ConditionalBuildSuccessful(conditionalUnit)
    local aiBrain = conditionalUnit.Brain --[[@as CampaignAIBrain]]
    local bManager = aiBrain.BaseManagers[conditionalUnit.BaseName]
    local selectedBuild = bManager.ConditionalBuildTable[bManager.ConditionalBuildManager.Index]
    local selectedData = selectedBuild.data
    -- Assign AI
    local newPlatoon = aiBrain:MakePlatoon('', '')
    aiBrain:AssignUnitToPlatoon(newPlatoon, conditionalUnit, 'Attack', 'None')
    newPlatoon:StopAI()
    newPlatoon:SetPlatoonData(selectedData.PlatoonData or {})

    if selectedData.PlatoonAIFunction then
        if type(selectedData.PlatoonAIFunction) == "function" then
            newPlatoon:ForkAIThread(selectedData.PlatoonAIFunction --[[@as function]])
        else
            newPlatoon:ForkAIThread(import(selectedData.PlatoonAIFunction[1])[selectedData.PlatoonAIFunction[2]])
        end
    end

    if selectedData.FormCallbacks then
        for _, callback in pairs(selectedData.FormCallbacks) do
    ...
    local cbManager = bManager.ConditionalBuildManager
    local waitTime = cbManager.WaitSecondsAfterDeath
    if waitTime then
        ...
        local name = selectedBuild.name
        ScenarioInfo.ConditionalBuildLocks[name] = true

        TriggerFile.CreateUnitDestroyedTrigger(function(unit)
            ForkThread(unlockConditionalBuildThread, name, waitTime)
        end, conditionalUnit)
    end

    -- Remove from the conditional build list if were not supposed to build any more
    if not selectedData.Amount then
        TableRemove(bManager.ConditionalBuildTable, cbManager.Index)
    elseif selectedData.Amount > 0 then
        -- Decrement the amount left to build
        selectedData.Amount = selectedData.Amount - 1

        -- If none are left to build, remove from the build table
        if selectedData.Amount == 0 then
            TableRemove(bManager.ConditionalBuildTable, cbManager.Index)
        end
    end
...
function DoConditionalBuild(singleEngineerPlatoon)
    ...
    local buildIndex = cbManager.Index
    ...
    if type(selectedBuild.name) == 'table' then
        ...
        if not unitToBuild then
            error('Unit with name "' .. unitName .. '" could not be found for conditional building.')
            return
        end
    else
        unitName = selectedBuild.name --[[@as string]]
        unitToBuild = ScenarioUtils.FindUnit(unitName, Scenario.Armies[aiBrain.Name].Units)
        if not unitToBuild then
            error('Unit with name "' .. unitName .. '" could not be found for conditional building.')
            return
    ...
    cbManager:OnUnitConstructionRequested(engineer, selectedBuild.data)
...
--#region Engineer Building Threads
...
function EngineerOnStartBuild(unit, unitBeingBuilt)
    ...
    elseif ExperimentalUnitName and bManager then
        ...
        cbManager:OnUnitConstructionStarted(unitBeingBuilt)
        ...
        if selectedBuild.data.KeepAlive then
            ...
            unitBeingBuilt.ConditionalBuildManager = cbManager
...
---@return boolean `True` if a structure was issued to be built, `false` if not
---@return string|nil `UnitName` of the structure that was issued to be built, or `nil` if no structure was issued
function BuildBaseManagerStructure(aiBrain, eng, baseManager, levelName, buildingType, platoon)
    local bmLevelName = baseManager.BaseName .. levelName
    local buildTemplate = aiBrain.BaseTemplates[bmLevelName].Template
    local buildList = aiBrain.BaseTemplates[bmLevelName].List

    if not buildTemplate or not buildList or not aiBrain:PlatoonExists(platoon) then
        return false
    end

    local namesTable = aiBrain.BaseTemplates[bmLevelName].UnitNames
    local buildCounter = aiBrain.BaseTemplates[bmLevelName].BuildCounter
    for _, entry in pairs(buildTemplate) do
        ---@type string
        local structureType = entry[1][1]
        -- Check if type (ex. T1AirFactory) is correct
        if (not buildingType or buildingType == 'ALLUNITS' or buildingType == structureType) and baseManager:CheckStructureBuildable(structureType) then
            ...
            if not (category and eng:CanBuild(category)) then
                continue
            end

            -- Iterate through the build locations to find the closest structure to our engineer position.
            local engPos = eng:GetPosition()
            local closest, closestPosition
            for i = 2, TableGetn(entry) do
                ---@type Vector
                local pos = entry[i]
                local position = Vector(pos[1], 0, pos[2])
                if aiBrain:CanBuildStructureAt(category, position) and baseManager:CheckUnitBuildCounter(pos, buildCounter) then
                    if not closest or Utilities.XZDistanceTwoVectorsSquared(engPos, position) < Utilities.XZDistanceTwoVectorsSquared(engPos, closestPosition) then
                        closest = pos
...
-- Default order of structures types for building
DefaultStructurePriorities = {
    'T3Resource',
    'T2Resource',
    'T1Resource',
    'T3EnergyProduction',
...
function BaseManagerEngineerThread(platoon)
    ...
    for _, v in pairs(platoon:GetPlatoonUnits()) do
        if not v.Dead and EntityCategoryContains(categoriesCONSTRUCTION, v) then
    ...
    local StructurePriorities = platoon.PlatoonData.StructurePriorities or DefaultStructurePriorities
    ...
    while aiBrain:PlatoonExists(platoon) do
        ...
        for _, levelData in ipairs(baseManager.LevelNames) do
            -- Loop through each priority category if the current build group has a higher build priority than 0
            if levelData.Priority <= 0 then continue end

            for _, v in ipairs(StructurePriorities) do
                local UnitType
                if v ~= 'ALLUNITS' then
                    UnitType = v
                end
    ...
    platoon:MoveToLocation(baseManager:GetPosition(), false)
end
...
function BuildUnfinishedStructures(platoon)
    ...
    repeat
        ...
        local buildingEngs = aiBrain:GetListOfUnits(categoriesENGINEER, false)
        ...
        for unitName, _ in pairs(bManager.UnfinishedBuildings) do
            local building = ScenarioInfo.UnitNames[armyIndex][unitName]
            if building and not building.Dead then
...
function TransportUnitsToLocation(platoon, finalLocation)
    ...
    local unit = units[1]
    if AIUtils.CheckUnitPathingEx(finalLocation, unit:GetPosition(), unit) then
...
--- Find an expansion base that needs engineers, and send them there.
--- Once they arrive, disband the platoon to the expansion base manager can grab them.
---@param platoon Platoon
function ExpansionEngineer(platoon)
    ...
    if not (data.BaseName and bManager and bManager.ExpansionBaseData) then
        aiBrain:DisbandPlatoon(platoon)
        return
    end

    ---@type PlatoonCommand|boolean
    local cmd = false
    for _, eData in ipairs(bManager.ExpansionBaseData) do
        ...
        if BMBC.NumEngiesInExpansionBase(aiBrain, data.BaseName, eData.BaseName) then
            ...
            eData.IncomingEngineers = eData.IncomingEngineers + TableGetn(platoon:GetPlatoonUnits())

            -- Remove engieneer from IncomingEngineers if it dies on the way
            platoon:AddDestroyCallback(ExpansionPlatoonDestroyed)

            local expansionPos = aiBrain.BaseManagers[eData.BaseName]:GetPosition()
            local currentPos = platoon:GetPlatoonPosition() --[[@as Vector]]
            if eData.TransportPlatoon or VDist3(currentPos, expansionPos) > 250 then
                cmd = TransportUnitsToLocation(platoon, expansionPos)
            end

            if not cmd then
                cmd = platoon:MoveToLocation(expansionPos, false)
            end
            break
        end
    end
    ...
    if cmd and type(cmd) ~= 'boolean' then
        while platoon:IsCommandsActive(cmd --[[@as PlatoonCommand]]) do
            WaitSeconds(5)
            if not aiBrain:PlatoonExists(platoon) then
                return
            end
...
function PermanentFactoryAssist(platoon)
    ...
    while aiBrain:PlatoonExists(platoon) do
        ...
        for _, v in pairs(facs) do
            if not v.Dead then
                local guards = v:GetGuards()
                local numGuards = 0
                for _, gUnit in pairs(guards) do
                    -- Make sure this guy is a permanent assister and not a transient assister
                    if not gUnit.Dead and not EntityCategoryContains(categoriesFACTORY, gUnit) and bManager.PermanentAssisters[gUnit] then
...
-- Default order of structure types for assisting
DefaultAssistCategories = {
    'MASSEXTRACTION',
    'MASSPRODUCTION',
    'ENERGYPRODUCTION',
    'FACTORY',
...
--- Assist construction projects in the base
---
--- Assist order:
--- - Other engineers building structures
--- - Factories producting units
---@param platoon Platoon
function BaseManagerAssistThread(platoon)
    ...
    local baseName = platoon.PlatoonData.BaseName
    local bManager = aiBrain.BaseManagers[baseName]
    ---@type BMAssistPlatoonData
    local assistData = platoon.PlatoonData.Assist
    local platoonPos = platoon:GetPlatoonPosition() --[[@as Vector]]
    ---@type Unit|nil
    local assistee
    local assistingBool = false
    local beingBuiltCategories = assistData.BeingBuiltCategories or DefaultAssistCategories

    local assistRange = assistData.AssistRange or bManager.Radius
    -- Faster to compare with squared value
    local assistRangeSqr = assistRange * assistRange
    local counter = 0
    local waitTime = 30

    while counter < (assistData.Time or 150) and aiBrain:PlatoonExists(platoon) do
        local guardedUnit = engineer:GetGuardedUnit()
        ...
        if not guardedUnit or (not guardedUnit:IsUnitState('Building') and not bManager:ConstructionNeedsAssister() and not bManager:IsConstructionUnit(guardedUnit)) then
            -- 1) Try to find a constructions engineer to assist to
            if bManager:ConstructionNeedsAssister() then
                local lowNum = 100000
                local highNum = 0
                ---@type Unit|nil
                local currLow
                for _, eng in pairs(bManager.ConstructionEngineers) do
                    local guardNum = TableGetn(eng:GetGuards())
                    if not eng.Dead and guardNum < lowNum then
                        currLow = eng
                        lowNum = guardNum
                    end
                ...
                if guardedUnit then
                    if guardedUnit.Dead or EntityCategoryContains(categoriesFACTORY, guardedUnit) or highNum > lowNum + 1 then
                        assistee = currLow
                    end
                else
                    assistee = currLow
            ...
            -- 2) Find any valid unit to assist. Get all units building stuff - TODO get list with point and radius; get list of units with state
            if not assistee then
                local unitsBuilding = aiBrain:GetListOfUnits(categoriesCONSTRUCTION, false)

                for _, buildeeCat in ipairs(beingBuiltCategories) do
                    ...
                    for _, unit in pairs(unitsBuilding) do
                        ...
                        if not buildingUnit or buildingUnit.Dead or not EntityCategoryContains(buildCat, buildingUnit) then continue end

                        -- If the unit building is a factory make sure its in the right PBM Location Type
                        if not EntityCategoryContains(categoriesFACTORY, unit) or aiBrain:PBMFactoryLocationCheck(unit, baseName) then
                            -- make sure unit is within valid assist range
                            local unitPos = unit:GetPosition()
                            if Utilities.XZDistanceTwoVectorsSquared(platoonPos, unitPos) < assistRangeSqr then
                                assistee = unit
                                break
                            end
                        end
            ...
            if assistee and not assistee.Dead then
                if EntityCategoryContains(categoriesFACTORY, assistee) then
                    platoon:Stop()
                    local guardee = assistee:GetGuardedUnit()
                    if guardee and not guardee.Dead and EntityCategoryContains(categoriesFACTORY, guardee) then
...
--#region Main Engineer Platoon Thread

local commanderCategory = categoriesCOMMAND + categoriesSUBCOMMANDER
local permAssisterCategory = categoriesENGINEER - commanderCategory
...
function BaseManagerSingleEngineerPlatoon(platoon)
    ...
    local cbManager = bManager.ConditionalBuildManager
    local unit = platoon:GetPlatoonUnits()[1]
    local canPermanentAssist = EntityCategoryContains(permAssisterCategory, unit)
    local commandUnit = EntityCategoryContains(commanderCategory, unit)
    ...
    while aiBrain:PlatoonExists(platoon) do
        if BMBC.BaseEngineersEnabled(aiBrain, baseName) then
            ...
            elseif canPermanentAssist and cbManager:NeedsMoreBuilders() then
...
--- Split the engineers platoon into single unit platoons
---
--- Used by the BaseManager platoon builders
---@param platoon Platoon
function BaseManagerEngineerPlatoonSplit(platoon)
    ...
    for _, engineer in pairs(units) do
        if engineer.Dead or not EntityCategoryContains(categoriesENGINEER, engineer) then
            continue
            -- Make sure current base manager isn't at capacity of engineers
        elseif bManager.CurrentEngineerCount >= bManager.EngineerQuantity then
            continue
        end

        if bManager.EngineerBuildRateBuff then
            Buff.ApplyBuff(engineer, bManager.EngineerBuildRateBuff)
        end

        local engPlat = aiBrain:MakePlatoon('', '')
        aiBrain:AssignUnitToPlatoon(engPlat, engineer, 'Support', 'None')
        ...
        if not EntityCategoryContains(commanderCategory, engineer) then
            bManager:AddCurrentEngineer()

            -- Only add death callback if it hasnt been set yet
            if not engineer.Subtracted then
                TriggerFile.CreateUnitDestroyedTrigger(BaseManagerSingleDestroyed, engineer)
            end
...
---@see OpAI.SetTargettingPriorities
---@param platoon Platoon
...
function GetScoutingPath(bManager, unit)
    local mapInfo = ScenarioInfo.MapData.PlayableRect or {
        0, 0, ScenarioInfo.size[1], ScenarioInfo.size[2]
    }

    local pathablePoints = {}
    local possiblePoints = {}
    if EntityCategoryContains(categoriesAIR, unit) and bManager:GetDefaultAirScoutPatrolChain() then
        -- Do air thing
        pathablePoints = ScenarioUtils.ChainToPositions(bManager:GetDefaultAirScoutPatrolChain() --[[@as string]])
    elseif EntityCategoryContains(categoriesLAND, unit) and bManager:GetDefaultLandScoutPatrolChain() then
        -- Do land thing
        pathablePoints = ScenarioUtils.ChainToPositions(bManager:GetDefaultLandScoutPatrolChain() --[[@as string]])
    else
...
function BaseManagerTMLAI(platoon)
    ...
    platoon:SetPrioritizedTargetList('Attack', DefaultTMLPlatoonPriorities)
...
function UnitUpgradeThread(unit)
    local aiBrain = unit:GetAIBrain() --[[@as CampaignAIBrain]]
    local baseName = unit.CDRData and unit.CDRData.BaseName
    if not baseName then return end
    local bManager = aiBrain.BaseManagers[baseName]
    if not bManager then return end

    -- Determine the type of unit
    local unitType
    if EntityCategoryContains(categoriesCOMMAND, unit) then
        unitType = 'DefaultACU'
    elseif EntityCategoryContains(categoriesSUBCOMMANDER, unit) then
    ...
    while not unit.Dead do
        ...
        if upgradeName and not unit:IsUnitState('Building') then
            ...
            aiBrain:AssignUnitToPlatoon(platoon, unit, 'Support', 'NoFormation')
            ...
            -- Walk back to the base, so the unit can't be grabbed by the base again.
            IssueToUnitMove(unit, bManager.Position)
            aiBrain:DisbandPlatoon(platoon)
        end
        WaitSeconds(5)
    end
