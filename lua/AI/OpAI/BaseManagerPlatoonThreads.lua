...
---@field ConditionalBuildData ConditionalBuildData # The conditional build data that spawned this unit
...
function CanConditionalBuild(singleEngineerPlatoon)
    ...
    if cbData.IsInitiated then
        ...
        if cbData.MainBuilder and not cbData.MainBuilder.Dead then
            return false
        end
    end
...
function ConditionalBuildSuccessful(conditionalUnit)
    ...
    local cbData = bManager.ConditionalBuildData
    local waitTime = cbData.WaitSecondsAfterDeath
    if waitTime then
    ...
    if not selectedData.Amount then
        TableRemove(bManager.ConditionalBuildTable, cbData.Index)
...
function DoConditionalBuild(singleEngineerPlatoon)
    ...
    local cbData = bManager.ConditionalBuildData
    cbData.MainBuilder = engineer
    cbData.NumAssisting = 1
    cbData.MaxAssisting = selectedBuild.data.MaxAssist or 1
    cbData.Unit = nil
    cbData.IsInitiated = true  -- Prevents other engineers from trying to start their own builds
    cbData.IsBuilding = false
    cbData.WaitSecondsAfterDeath = selectedBuild.data.WaitSecondsAfterDeath
    ...
    while counter < (assistData.Time or 150) and aiBrain:PlatoonExists(platoon) do
        ...
        if not guardedUnit or (not guardedUnit:IsUnitState('Building') and not bManager:ConstructionNeedsAssister() and not bManager:IsConstructionUnit(guardedUnit)) then
            ...
            if bManager:ConstructionNeedsAssister() then
                ...
                if guardedUnit then
                    if guardedUnit.Dead or EntityCategoryContains(categories.FACTORY, guardedUnit) or highNum > lowNum + 1 then
                        assistee = currLow
                    end
                else
                    assistee = currLow
            ...
            if not assistee then
                ...
                for _, buildeeCat in ipairs(beingBuiltCategories) do
                    ...
                    for _, unit in pairs(unitsBuilding) do
                        ...
                        if not EntityCategoryContains(categories.FACTORY, unit) or aiBrain:PBMFactoryLocationCheck(unit, baseName) then
            ...
            if assistee and not assistee.Dead then
                if EntityCategoryContains(categories.FACTORY, assistee) then
                    platoon:Stop()
...
local commanderCategory = categories.COMMAND + categories.SUBCOMMANDER
local permAssisterCategory = categories.ENGINEER - commanderCategory
...
function BaseManagerEngineerPlatoonSplit(platoon)
    ...
    for _, engineer in pairs(units) do
        if engineer.Dead or not EntityCategoryContains(categories.ENGINEER, engineer)then
            continue
