...
local LOG = LOG
local debugAllocinfo = debug.allocinfo
...
function checkpoint_to_table(checkpoint)
    ...
    local r = {}
    local j = 0
    for i=1,tableGetn(checkpoint),7 do
        j = j+1
        r[j] = {
            func = checkpoint[i],
...
function memreport()
    local all = tableCopy(debugAllocinfo())
    ...
    for obj,trace in all do
        ...
        sum.bytes = sum.bytes + debugAllocatedsize(obj)
    end

    rt = tableValues(rt)
    tableSort(rt, sort_down_by 'bytes')
