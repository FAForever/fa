...
- Blueprint ID: Blueprint ID of the unit in lowercase.
...
When the same change has been applied to multiple units, you can just write the group in the title. The unitID still loads the unit icone, so choose a unit that is most represantative. (UEF when all factions are affected).
...
- Further increase the MuzzleSalvoSize of the Cybran ACU's Nanite Torpedo upgrade to 4, as it still had difficulties penetrating torpedo defenses after (#6476) increased it to 3. Its DPS remains unchanged (#6542).
    - Nanite Torpedo Launcher (enhancements/cybran/torp)
        - Torpedo weapon:
