---
layout: post
title: 3836 - Game Patch
permalink: changelog/3836
---

# Game version 3836 (May 15, 2026)

This patch is mainly a balance patch with one notable exception: Thanks to Saver's tireless work we now have new HQ and support factory designs that improve on the previous designs as well as fixing many technical issues. Please let him know if there are any concerns left that need addressing.
...
- (#7074, #7117) Add preliminary changelogs of fafdevelop and fafbeta to the changelog window in the lobby.
  These changelogs will be updated automatically with each deployment, allowing you to easily understand what exactly has changed when playing these game modes. You need to host a lobby in the appropriate game mode to see these changelogs. Alternatively, you can also find them on the [changelog website](https://faforever.github.io/fa/changelog).
...
- (#7108) Decrease Seraphim cruiser's missile damage and splash radius.
  ...
  The TML rework gave Seraphim fast missiles as a faction trait which caused their high DPS cruiser missiles to start hitting navy. While hitting navy is interesting, the DPS was not balanced for this, so the damage and splash are reduced. The Seraphim carrier keeps its 250 DPS missile launcher since it costs twice as much.
...
- (#7020, [#132](https://github.com/FAForever/FA-Binary-Patches/pull/132)) Improve pathfinding by disabling internal intermediary waypoints for long-distance moves that lead to units moving in columns.
...
- ([#143](https://github.com/FAForever/FA-Binary-Patches/pull/143), [#157](https://github.com/FAForever/FA-Binary-Patches/pull/157)) Fix units constructed earlier than their target aiming too far ahead on the moving target. Especially with beam weapons aiming at small units this could lead to misses.
- ([#145](https://github.com/FAForever/FA-Binary-Patches/pull/145)) Account for collision box offset when picking target point of a unit with no target bones.

## UI

- ([#147](https://github.com/FAForever/FA-Binary-Patches/pull/147)) Adds console commands for scaling unit, projectile and deposit icons. Each set of icons has its own scale value.
  * `ui_DepositIconScale`
