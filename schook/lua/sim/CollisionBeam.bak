...
CollisionBeam = Class(moho.CollisionBeamEntity) {
	...
	DoReverseBeam = function(self, instigator, targetEntity)
		#WARN("doing reverse beam")
		...
		if targetEntity then
			for f, g in self.FxBeam do	           
				LOG('CREATING REVERSE BEAM !!!')
				local beam = AttachBeamEntityToEntity(self, 0, targetEntity, 0, army, self.FxBeam[Random(1, table.getn(self.FxBeam))])                         
    ...
    OnImpact = function(self, impactType, targetEntity)
		...
		if not self.Weapon:GetBlueprint().ContinuousBeam then #and self.Weapon.GetBlueprint().ContinuousBeam == 'true'
			self:DoReverseBeam(instigator, targetEntity)
			#LOG("Beam self info is " .. repr(self))	
		#else 
		#	self:CreateBeamEffects() 
		end
